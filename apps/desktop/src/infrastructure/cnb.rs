use super::issues::{ActionResult, Request, Response};
#[cfg(test)]
use crate::model::issues::Issue;
use crate::{
    i18n::LocalizedText,
    model::issues::{
        Cli, Comment, IssueAction, IssueFilter, IssuePage, IssueProvider, PAGE_SIZE, User,
    },
};
        ...
        Request::Detail {
            ...
            let result = (|| {
                runtime
                    .as_ref()
                    .map_err(|error| anyhow::anyhow!(error.to_string()))?
                    .block_on(issue_request(&cli, &repository, &number, "get-issue", &[]))
            })()
            .map_err(localized_error);
...
pub(crate) fn repository_from_remote(remote: &str) -> Option<String> {
    super::issues::repository_from_remote(IssueProvider::Cnb, remote)
}

fn repository_from_remotes(remotes: &str) -> Option<String> {
    super::issues::repository_from_remotes(IssueProvider::Cnb, remotes)
}
...
pub(super) async fn run_with_temp_dir(
    ...
    super::issues::run_cli(IssueProvider::Cnb, path, args, timeout, directory).await
}
