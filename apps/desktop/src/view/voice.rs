...
impl NexusView {
    ...
    pub(super) fn render_voice_controls(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let label = if voice.operation.is_some() {
            "停止录音"
        } else if voice.ready() {
            "语音输入"
        } else {
        ...
        div().debug_selector(|| "voice-settings".into()).flex().flex_col().gap_8()
            ...
            .child(div().flex().gap_2().children(voice::supported_providers().iter().copied().map(|provider| {
                Button::new(match provider { Provider::MacOs => "voice-macos", Provider::Mimo => "voice-mimo" })
                    ...
                    .label(match provider { Provider::MacOs => "macOS 系统语音识别", Provider::Mimo => "MiMo ASR" })
            ...
            .when(selected.is_none(), |el| el.child(div().text_size(px(12.)).text_color(rgb(colors.muted)).child("尚未选择 Provider。")))
            ]))
            .when(selected == Some(Provider::Mimo), |el| el.child(
                settings_group(colors, "MiMo ASR", [
                div().debug_selector(|| "voice-mimo-config".into()).py_4().flex().flex_col().gap_4()
                    .child(if voice.mimo_configured { "已配置（系统凭据库）；不代表服务请求已验证。" } else { "待配置：保存 API Key 后才能录音。" })
                    .child(div().text_size(px(12.)).text_color(rgb(colors.muted)).line_height(relative(1.6))
                        .child("停止录音后，音频将发送给 MiMo。最长 60 秒，Base64 音频不超过 10 MB。保存 Key 不会上传音频。"))
                    .child(Input::new(&self.voice_key_input))
                    .child(Button::new("voice-save-key").primary().small().h(px(CONTROL_HEIGHT)).label("保存 API Key").on_click(cx.listener(|this, _, window, cx| {
            ...
            .when(selected == Some(Provider::MacOs), |el| el.child(
                settings_group(colors, "macOS 系统语音识别", [
                div().py_4().flex().flex_col().gap_4()
                    .child(div().text_size(px(13.)).text_color(rgb(colors.muted)).line_height(relative(1.6))
                        .child("无需 API Key。首次录音时请求麦克风和 Speech 权限；拒绝后请在系统设置中恢复。"))
                    .child(div().text_size(px(12.)).text_color(rgb(colors.muted)).line_height(relative(1.6))
                        .child("系统识别可能需要联网，音频可能发送给 Apple；每段最多 60 秒。"))
                    .child(div().text_size(px(13.)).line_height(relative(1.6)).child(voice::native_status(voice.settings.locale.as_deref())))
                    .child(Button::new("voice-locale").outline().small().h(px(CONTROL_HEIGHT)).label(voice.settings.locale.clone().unwrap_or_else(|| "跟随系统语言".into()))
                        .dropdown_menu({ let view = cx.entity().downgrade(); move |menu, _, _| {
                            ...
                            for locale in std::iter::once(None).chain(voice::native_locales().into_iter().map(Some)) {
                                let view = view.clone();
                                menu = menu.item(PopupMenuItem::new(locale.clone().unwrap_or_else(|| "跟随系统语言".into()))
                    ...
                    .child(Button::new("voice-permissions-refresh").ghost().small().h(px(CONTROL_HEIGHT)).icon(IconName::RotateCw).label("刷新权限状态").on_click(cx.listener(|_, _, _, cx| cx.notify())))
                ])
            ))
            .when(!voice.status.is_empty(), |el| el.child(div().text_size(px(13.)).text_color(rgb(colors.text_secondary)).child(voice.status.clone())))
    }
}
