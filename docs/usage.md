...
在 **设置 → Source Control → CNB** 中启用集成并检测本机 [CNB CLI](https://docs.cnb.cool/en/develops/cnb-cli.html)。先在终端安装 CLI、运行 `cnb login` 登录，账号需要目标仓库的 Issue 读取权限。Nexus 使用 CLI 已有凭据，只保存集成开关。
...
请求在后台执行；未安装 CLI、登录或权限问题、网络超时均会提示重试或配置入口。浏览 CNB 不会创建会话，也不会中断正在运行的任务。本次集成仅提供读取，不包含 Issue 编辑、评论发布或 PR 管理。
