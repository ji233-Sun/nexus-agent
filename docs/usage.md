...
引擎和模型就绪后，直接发送第一条消息即可创建任务，无需先选择文件夹。未关联项目的对话显示在侧栏「未关联项目」下；点击该入口或在输入框上方的项目选择器中选择「未关联项目」，可开始新的独立对话。需要处理项目文件时，仍可通过「添加本地项目」选择目录。切换项目上下文会准备新任务，已有对话保留原来的目录绑定。

时间线显示回答、工具调用、状态和错误，任务标题会异步生成；标题生成失败时，保留首条消息的本地回退标题。
...
输入框旁的 **附件** 按钮支持多选文件，也可以将文件拖入输入框，或直接粘贴剪贴板中的图片／文件。图片显示可点击放大的缩略图，其他文件显示文件卡片，点击卡片可在文件管理器中定位副本。发送前可以逐个移除；只有附件、没有文字时也可以发送。

PNG、JPEG、GIF、WebP 按图片发送到 **Codex／Claude Code** 的视觉输入通道，需选择支持视觉的模型。其他格式（包括 PDF、文本、表格、压缩包、SVG 等）作为普通文件，向各个 Harness 提供本地副本路径，由 Agent 按需读取；是否能解析取决于所选 Agent 的工具和权限。不会自动展开压缩包或把文件全文塞入消息。

每条消息最多 8 个附件；单张图片最多 5 MiB，其他单个文件最多 100 MiB，不接受文件夹。导入在后台完成，期间暂不可发送。Nexus 保存附件的独立副本，因此移动或修改原文件不会影响聊天附件；已发送的附件随历史记录保留，可用“再次加入聊天”重用。发送失败会保留草稿，含附件的排队消息在下一轮发送。不支持图片的 Harness 会提示切换，不会静默丢弃图片。

## PDF 阅读与标注

点击输入框旁的 **PDF** 按钮选择本地文件，也可以将 PDF 拖入输入框以外的工作区，或点击聊天里的本地 PDF 链接。文档在 Nexus 的独立窗口中打开，可与聊天同时查看；支持连续滚动、页码跳转、缩略图、目录、缩放及文本选择／复制／搜索。单个文档最多 100 MiB。

工具栏可添加高亮、下划线、画笔、矩形、箭头、文本或批注；切换到“选择与移动标注”后可移动、改色、删除或编辑文字。页面支持旋转、前移／后移和删除，编辑支持撤销／重做。第一次“保存”会询问保存位置，默认文件名为 `原文件名-annotated.pdf`，后续保存写入所选文件；“另存为”可选择另一份副本。关闭未保存文档时会提示。

新增标注在导出时合并为页面内容，其他阅读器可显示；原页面文字仍可选择。重开导出文件后可添加新标注，已合并的标注无法再单独修改。加密 PDF 输入密码后可预览和截图，暂不支持编辑保存；原正文改写、数字签名和交互表单编辑不在当前范围。

“整页加入聊天”直接截取当前页；“框选加入聊天”在页面上拖动选择区域。截图包含标注，草稿显示缩略图、来源文件名与原始页码，点击缩略图可放大，发送前可移除或补充问题。截图只包含文档区域，每张最多 5 MiB、与其他附件合计每条消息最多 8 个，随已发送聊天记录保存。含截图的排队消息在下一轮发送；消息下方的“再次加入聊天”可以取回截图重新编辑和发送。

图片输入目前接入 **Codex 和 Claude Code**，需选择支持视觉的模型。其他 Harness 会提示切换配置，并保留草稿；模型拒绝图片时可从消息记录取回截图。文件预览、标注和截图在本机完成，发送聊天后按所选 Harness／Provider 的正常流程传给模型。

Linux 的内置 PDF 窗口需要 WebKitGTK 4.1：Ubuntu／Debian 运行时安装 `libwebkit2gtk-4.1-0`，构建依赖见开发指南。
...
在 **设置 → Source Control → CNB** 中启用集成并检测本机 [CNB CLI](https://docs.cnb.cool/en/develops/cnb-cli.html)。先在终端安装 CLI、运行 `cnb login` 登录，账号需要目标仓库的 Issue、评论读取权限；指派、状态变更和 NPC 委托还需要对应的写入权限。Nexus 使用 CLI 已有凭据，只保存集成开关。

打开 Git 项目后，Nexus 自动识别指向 `cnb.cool` 的 HTTPS / SSH remote；优先选择 `origin`，也支持其他 remote 和多级组织。识别成功后，项目名与第一条会话之间会出现带 CNB 图标的入口。修改 remote 或安装 CLI 后，可从设置中「重新检测」。

点击 **CNB → Issue**，按更新时间浏览「未关闭 / 已关闭」列表，每页 30 条，使用上一页、下一页访问全部结果。列表显示编号、标题、作者、标签、优先级、更新时间和评论数；点击后可查看 Markdown 描述、处理人及创建时间，并可在 CNB 中打开原始 Issue。返回列表会保留筛选、页码和滚动位置。

详情页会读取全部评论，并保留 Markdown 和图片、音视频附件展示。可使用以下操作：

- **快捷提 Issue**：在列表工具栏选择 Harness、模型、思考档位和权限模式，填写要提交的 Issue 内容后直接启动 Harness 创建 Issue；创建过程作为普通对话显示在侧栏。Harness 使用本机已登录的 `gh` / `cnb` CLI 创建，创建完成后在对话中返回 Issue 链接。
- **用 AI 处理**：在弹窗中选择 Harness、模型、思考档位和权限模式，可填写「补充信息（选填）」；确认后直接启动 Harness，将 Issue 链接、标题、描述、作者、处理人、标签、优先级、时间及全部评论连同补充信息作为任务发送，并作为普通对话显示在侧栏。评论尚未完整读取时不可启动。
- **指派给我**：查询当前 CNB CLI 登录用户，将其添加为处理人，并保留已有处理人。
- **关闭 Issue / 重新打开 Issue**：同步更新详情和当前筛选列表，返回列表时刷新分页数据。
- **CodeBuddy NPC**：向当前 Issue 发布 `@CodeBuddy` 评论，使用系统内置 NPC 的工作模式（需要仓库开发者及以上权限）完成开发、验证并创建 PR。随后显示该评论对应的 **Action** 链接；链接尚未生成或查询失败时，可刷新 NPC 状态，刷新不会重复发布评论或触发任务。
...
OMP 使用 `rpc-ui` 模式向内置 `ask` 工具提供交互能力，需要安装支持该模式的 OMP。选择题保留选项说明，将「Other (type your own)」显示为「其他回答」输入框，直接填写并提交即可，无需再回答第二个编辑器问题；多选题按 OMP RPC 的逐项选择与完成流程回答。提示文字和原始预填内容显示在问题中，不自动填入回答；请求按原生 timeout 过期。OMP 的原生工具授权（`Allow tool: …`，`Approve` / `Deny`）仍显示审批弹窗。
...
macOS 还在菜单栏常驻应用图标（Status Item），点击后可显示或隐藏主窗口、新建任务或退出应用；`⌘ W` 与菜单栏的「隐藏主窗口」会隐藏整个应用，窗口、草稿和后台任务都保留，点击菜单栏图标即可恢复。`⌘ Q`、菜单栏和状态栏菜单的「退出 Nexus Agent」在存在运行中任务时会先确认。Windows 的 `Alt + F4` 与标题栏关闭按钮遵循系统惯例退出应用，同样会在有任务运行时先确认。Linux 关闭窗口的行为与之前一致。
...
未关联项目的任务使用数据库旁的 `sessions/<任务 ID>` 独立目录，模型发现与各轮运行共用该目录。重启后可从侧栏继续查看和续聊；不同任务不共享工作目录，也不会沿用上一个项目的路径。悬停输入框上方的「未关联项目」可查看实际路径。归档或删除对话不会删除目录中的文件；若目录已被外部移除，续聊会提示错误。
...
### Kimi Code、Qoder、CodeBuddy、OpenCode 与 DeepSeek Harness

Kimi Code 和 OpenCode 只支持 ACP，新会话固定使用 ACP 接入。DeepSeek Harness（`dsh`）同样只使用 ACP，固定以 `dsh --profile acp` 接入。Qoder / Qoder CN / CodeBuddy 新会话默认使用原生 stream-json，设置中的「接入方式」可以切换到 ACP，均支持流式回答、工具结果、审批和原生会话续聊。先在各自 CLI 完成登录或配置：`kimi login`、`qoder login`、`qodercn login`、`codebuddy login`、`opencode auth login`；dsh 无需 CLI 登录，模型调用读取 `DEEPSEEK_API_KEY`（可用 `DEEPSEEK_BASE_URL` 覆盖端点），也可以先运行 `dsh web` 在设置中保存 API Key。
...
| DeepSeek Harness | `@deepseek-ai/dsh`，命令 `dsh`（开发者预览版） | 安装菜单通过 npm 安装；ACP 不提供 `session/set_mode`，Nexus 会跳过模式恢复，工具审批每次都走一次性放行/拒绝。`dsh` 处于开发者预览期，协议可能随版本变化 |
...
模型和思考层级仍通过 ACP 从当前 CLI 获取。Kimi Code 和 OpenCode 需要原生登录状态，仅填写 Provider API Key 不能替代探测的登录检查；探测与刷新目录不会调用模型，但 CLI 可能保存空会话。OpenCode 的模型目录沿用 CLI 的 `provider/model` 标识；由于 ACP 会话新建时默认落在 `build` 模式，Nexus 会显式恢复该模式，OpenCode 没有其他引擎使用的 `default` 模式。DeepSeek Harness 的模型目录以 `["provider","model"]` 不透明组合值展示（默认 `deepseek-official` 路由的 `deepseek-flash` 与 `deepseek-v4-pro`），思考层级为 `off / low / high / max`。

接入方式设置影响新会话；已有会话沿用创建时的协议，旧版本的未标记会话继续使用 ACP。Kimi Code 只使用 ACP，旧版 `kimi-cli` Wire 会话无法恢复，需升级到 kimi-code 后重新开始会话；OpenCode 与 DeepSeek Harness 同样只使用 ACP。CodeBuddy 新会话同时记录显式选择的地区，续聊时恢复该地区。原生 Session 必须仍然存在。ACP v1 的厂商私有 User Ask 扩展尚未接入。
