...
点击输入框旁的 **PDF** 按钮选择本地文件，也可以将 PDF 拖入工作区，或点击聊天里的本地 PDF 链接。文档在 Nexus 的独立窗口中打开，可与聊天同时查看；支持连续滚动、页码跳转、缩略图、目录、缩放及文本选择／复制／搜索。单个文档最多 100 MiB。
...
“整页加入聊天”直接截取当前页；“框选加入聊天”在页面上拖动选择区域。截图包含标注，草稿显示缩略图、来源文件名与原始页码，点击缩略图可放大，发送前可移除或补充问题。截图只包含文档区域，每张最多 5 MiB、每条消息最多 8 张，随已发送聊天记录保存。含截图的排队消息在下一轮发送；消息下方的“再次加入聊天”可以取回截图重新编辑和发送。
