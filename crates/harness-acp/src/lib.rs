...
pub fn prepare_kimi_title(
    run: &StartRun,
    prompt: &str,
    cwd: &Path,
) -> Result<(LaunchSpec, KimiTitleDecoder), String> {
    let directory = tempfile::tempdir().map_err(|_| "无法创建 Kimi 标题配置。")?;
    std::fs::write(
        directory.path().join("system.md"),
        "Return only a short task title.",
    )
    .map_err(|_| "无法写入 Kimi 标题提示。")?;
    std::fs::write(directory.path().join("agent.yaml"), "version: 1\nagent:\n  name: nexus-title\n  system_prompt_path: system.md\n  tools: []\n  subagents: {}\n").map_err(|_|"无法写入 Kimi 标题配置。")?;
...
pub struct KimiTitleDecoder {
    _directory: tempfile::TempDir,
    text: String,
}
impl LineDecoder for KimiTitleDecoder {
    fn decode_line(&mut self, line: &str) -> Result<Vec<DecodedEvent>, serde_json::Error> {
...
mod tests {
    ...
    fn kimi_title_agent_has_no_tools_or_mcp_and_never_resumes_conversation() {
        ...
        let (spec, mut decoder) =
            prepare_kimi_title(&run, "title prompt", Path::new("/project")).unwrap();
